...
app.post('*', (req, res) => {
  ...
  const result = request('http://192.168.30.146:3000/users/'.concat(phoneNumber.replace('+', '')), function (error, resp, body) {
      console.log(error)
      //console.log(resp)
    ...
    if (resp.statusCode == 200) {
        ...
        if (text == '' || text.endsWith('*9')) {            
            ...
            request('http://192.168.30.146:3000/invoices/1/'.concat(clientID), function (errorInv, resInv, bodyInv) {
            ...
            request('http://192.168.30.146:3000/invoices/5/'.concat(clientID), function (errorInv, resInv, bodyInv) {
                const data = bodyInv
            ...
            //Business logic for the second level response
            //Fetch current Invoice for the user's ClientID
            request('http://192.168.30.146:3000/invoices/bal/'.concat(clientID), function (errorInv, resInv, bodyInv) {
          ...
          res.send(response)
            ...
            let options = {
                'method': 'POST',
                'url': 'http://192.168.30.146:3000/readings/',
                'headers': {
        ...
        /*    request('http://192.168.30.248/api/getCoupon?clientNumber='.concat(clientID), function (errorInv, resInv, bodyInv) {
            ...
            request('http://192.168.30.146:3000/promotions/'.concat(clientID), function (errorInv, resInv, bodyInv) {
		response = `END `
                res.send(response.concat(bodyInv))
                /*if (resInv.statusCode == 200) {
                    response = `END O saldo do cliente `.concat(clientID, ' é: \n\r', bodyInv, '\n\rObrigado!')
                    res.send(response)
                } else if (resInv.statusCode == 404) {
            ...
            request('http://192.168.30.146:3000/promotions/', function (errorInv, resInv, bodyInv) {
                response = `END `
		res.send(response.concat(bodyInv))
            /*    if (resInv.statusCode == 200) {
                    response = `END O saldo do cliente `.concat(clientID, ' é: \n\r', bodyInv, '\n\rObrigado!')
        ...
        if (text == '') {
            ...
            request('http://192.168.30.146:3000/clients/'.concat(clientID, '/', entityID), function (errCli, resCli, bodyCli) {
                console.log(resCli.statusCode);

                if (!errCli && resCli.statusCode == 200) {
                    let options = {
                        'method': 'POST',
                        'url': 'http://192.168.30.146:3000/users/',
                        'headers': {
                          'Content-Type': 'application/x-www-form-urlencoded'
                        },
                        form: {
